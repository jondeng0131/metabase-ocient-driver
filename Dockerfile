...
COPY deps.edn /build/driver/
COPY src/ /build/driver/src
COPY resources/ /build/driver/resources
...
COPY --from=stg_build /build/driver/target/ocient.metabase-driver.jar /
...
COPY --chown=2000:2000 --from=stg_build \
    /build/driver/target/ocient.metabase-driver.jar \
